...
class SaveChunkTest:
    ...
    def get_excel_file_dir_list(self):
    ...
    def get_file_list(self, excel_file_dir: str) -> list[str]:
        file_list = []
    ...
    def get_documents_length_by_news_url(self, url):
        try:
            # Document Load
    ...
    def load_data(self, day, topic, news_count, data):
        ...
        for idx, url  in enumerate(news_data_url):
            ...
            if pd.notna(url):  # Check if url is not NaN
                try:
                    # Document Loader
                    news_length = self.get_documents_length_by_news_url(url)
    ...
    def chunk_list_check(self):
        excel_file_dir_list = self.get_excel_file_dir_list()
        # 1. 날짜 디렉토리
        for idx, excel_file_dir in enumerate(excel_file_dir_list):        
            ...
            # 2. 날짜별로 주간 이슈 엑셀 파일
            daily_topic_list = self.get_file_list(excel_file_dir)
            ...
            for file_name in (daily_topic_list):
                ...
                news_count = len(data)
                self.load_data(day=news_day, topic=file_name, news_count=news_count, data=data)
...
if __name__ == "__main__":
    ...
    save_result =  chunkTest.chunk_list_check()
    if save_result == 1:
